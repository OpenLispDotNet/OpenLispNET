    ...
    public unsafe class DirectBitmap
    {
        ...
        public void DrawChar(char c, Font font, int color, int x, int y)
        {
            ...
            for (int i = 0; i < height; i++)
            {
                ...
                {
                    if (font.ConvertByteToBitAddres(data[num + i], b + 1))
                    {
        ...
        public void DrawImage(Bitmap image, int x, int y)
        {
            for (int yi = 0; yi < image.Height; yi++)
            {
                ...
                int[] aDestIntArray = Bitmap.rawData;
                // Calculate the total number of bytes required
                int aDestByteCount = aDestIntArray.Length * sizeof(int);
                // Create a byte array to hold the result
                byte[] aDestByteArray = new byte[aDestByteCount];
                // Use Buffer.BlockCopy to perform the conversion
                Buffer.BlockCopy(aDestIntArray, 0, aDestByteArray, 0, aDestByteCount);

                int[] aSrcIntArray = image.rawData;
                // Calculate the total number of bytes required
                int aSrcByteCount = aDestIntArray.Length * sizeof(int);
                // Create a byte array to hold the result
                byte[] aSrcByteArray = new byte[aDestByteCount];
                // Use Buffer.BlockCopy to perform the conversion
                Buffer.BlockCopy(aDestIntArray, 0, aDestByteArray, 0, aDestByteCount);

                //MemoryOperations.Copy(Bitmap.rawData, destOffset, image.rawData, srcOffset, count);
                MemoryOperations.Copy(aDestByteArray, destOffset, aSrcByteArray, srcOffset, count);
            }
        }
        ...
        public Bitmap ExtractImage(int srcX, int srcY, int width, int height)
        {
            ...
            for (int yi = 0; yi < height; yi++)
            {
                ...
                int[] aSrcIntArray = Bitmap.rawData;
                // Calculate the total number of bytes required
                int aSrcByteCount = aDestIntArray.Length * sizeof(int);
                // Create a byte array to hold the result
                byte[] aSrcByteArray = new byte[aDestByteCount];
