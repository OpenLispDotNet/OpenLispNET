...
namespace OpenLisp.Core.Kernel.OS.System.Processing.Applications.Emulators.GameBoyEmu.DMG
{
    public class PPU
    {
        ...
        public void update(int cycles, MMU mmu)
        {
            ...
            if (isLCDEnabled(mmu.LCDC))
            {
                ...
                { //handle coincidence Flag
                    mmu.STAT = BitOps.bitSet(2, mmu.STAT);
                    if (BitOps.isBit(6, mmu.STAT))
                    {
                ...
                {
                    mmu.STAT = BitOps.bitClear(2, mmu.STAT);
                }
        ...
        private void changeSTATMode(int mode, MMU mmu)
        {
            ...
            else if (mode == 0 && BitOps.isBit(3, STAT))
            { // Bit 3 - Mode 0 H-Blank Interrupt     (1=Enable) (Read/Write)
                mmu.requestInterrupt(LCD_INTERRUPT);
            }

            //VBLANK - Mode 1 (4560 cycles - 10 lines)
            else if (mode == 1 && BitOps.isBit(4, STAT))
        ...
        {
            byte LCDC = mmu.LCDC;
            if (BitOps.isBit(0, LCDC))
            { //Bit 0 - BG Display (0=Off, 1=On)
                renderBG(mmu);
            }
            if (BitOps.isBit(1, LCDC))
        ...
        private ushort getBGTileMapAdress(byte LCDC)
        {
            ...
            return BitOps.isBit(3, LCDC) ? (ushort)0x9C00 : (ushort)0x9800;
        }
        ...
        private ushort getWindowTileMapAdress(byte LCDC)
        {
            ...
            return BitOps.isBit(6, LCDC) ? (ushort)0x9C00 : (ushort)0x9800;
        }
        ...
        {
            //Bit 4 - BG & Window Tile Data Select   (0=8800-97FF, 1=8000-8FFF)
            return BitOps.isBit(4, LCDC) ? (ushort)0x8000 : (ushort)0x8800; //0x8800 signed area
        }
        ...
        private void renderSprites(MMU mmu)
        {
            ...
            { //0x9F OAM Size, 40 Sprites x 4 bytes:
                ...
                {
                    byte palette = BitOps.isBit(4, attr) ? mmu.OBP1 : mmu.OBP0; //Bit4   Palette number  **Non CGB Mode Only** (0=OBP0, 1=OBP1)
        ...
        private int spriteSize(byte LCDC)
        {
            ...
            return BitOps.isBit(2, LCDC) ? 16 : 8;
        }
        ...
        private bool isWindow(byte LCDC, byte WY, byte LY)
        {
            ...
            return BitOps.isBit(5, LCDC) && WY <= LY;
        }
    }
}
